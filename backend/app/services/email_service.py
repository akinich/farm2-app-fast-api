...
async def test_email_connection(conn: Connection, test_email: str) -> Dict[str, Any]:
    ...
    try:
        ...
        await _send_email_via_provider(
            ...
            provider=provider,
            to_email=test_email,
            subject="Email Test - Farm ERP",
            plain_body=f"This is a test email from your Farm ERP.\n\nProvider: {provider}\n\nEmail service is configured correctly!",
            html_body=f"<h2>Email Test</h2><p>This is a test email from your Farm ERP.</p><p><strong>Provider:</strong> {provider}</p><p><strong>Email service is configured correctly!</strong></p>"
        )
...
async def _get_smtp_settings(conn: Connection) -> Dict[str, Any]:
    ...
    return {
        ...
        'from_name': await settings_service.get_setting(conn, 'email.from_name', 'Farm ERP'),
    }
